...
def home(request):
    ...
    for item in procurement_items:
        procurement_data.append([item.item_name, item.quantity or 0])

    procurement_chart_options = {
    ...
    # Prepare data for dolls stock chart - showing current stock for each doll
    dolls_data = []
    doll_inventory = Stock.objects.all().order_by("item_name")

    for doll in doll_inventory:
        dolls_data.append([doll.item_name, doll.total_quantity])
    ...
    context = {
        ...
        "procurement_chart_options": procurement_chart_options,
        "dolls_chart_options": dolls_chart_options,
    }
...
def sitter_arrival_delete(request, sitter_id):
    ...
    if request.method == "POST":
        ...
        messages.success(request, "Sitter deleted successfully from arrival list.")
...
def sitter_arrival_list(request):
    ...
    return render(
        request,
        "sitter_arrival_list.html",
        {"sitter_arrival_list": sitter_arrival_list},
    )
...
def baby_signedoutlist(request):
    ...
    return render(
        request, "baby_signedoutlist.html", {"baby_signedoutlist": baby_signedoutlist}
    )
...
def profile(request):
        ...
        if "password_change" in request.POST:
            ...
            if password_form.is_valid():
                ...
                update_session_auth_hash(
                    request, user
                )  # Important to keep user logged in
    ...
    return render(
        request, "profile.html", {"password_form": password_form, "user": request.user}
    )
...
def update_admin(request):
    if request.method == "POST":
        ...
        try:
            ...
            admin.save()
            return HttpResponse("Admin user updated successfully! Please log in again.")
